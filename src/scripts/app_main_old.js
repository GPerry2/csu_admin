...
*/
  config =  {
    "title": "Contractor Status Update",
    "default_repo": "csu",
    "upload_repo": "csu_update",
    "default_view": "vendor",
    ...
    "upload_defaults": {
      ...
      "addRemoveLinks": true,
      "dictMaxFilesExceeded": "Maximum 5 uploaded files"
    },
    "repo": {
      "vendor": "admin/vendor",
    ...
    "formName": {
      ...
      "user_status": "System User Status Profile",
      "bu": "Business Unit Profile",
      "rd": "Reference Data",
      "bu_it": "Business Unit Invoicing Text"
    },
    "formHeaderFieldMap": {
      "vendor": "VendorName",
      "user_status": "System User Status Profile",
    ...
    "listName": {
      ...
      "user_status": "System User Status Profiles",
      "bu": "Business Unit Profiles",
      "rd": "Reference Data",
      "bu_it": "Business Unit Invoicing Texts"
    },
    "button": {
      "createNewEntry": "Create New Vendor",
      "viewSubmissions": "Back to Work Orders",
      "print": "Print",
    ...
    "api": {
      "get": "/c3api_data/v2/DataAccess.svc/",
      "post": "/c3api_data/v2/DataAccess.svc/",
      "put": "/c3api_data/v2/DataAccess.svc/",
      "delete": "/c3api_data/v2/DataAccess.svc/",
      "email": "/cc_sr_admin_v1/submit/csu_email",
      "eventDispatcher": "/rest/COTEventDispatcher_V2/REST",
      "vendor_login": "/cc_sr_v1/submit/",
    ...
    "breadcrumbtrail": [{
    ...
    },
      {
        ...
        "link": "http://www1.toronto.ca/wps/portal/contentonly?vgnextoid=65aae03bb8d1e310VgnVCM10000071d60f89RCRD"
      }
    ],
    "top_menu": [{
      "title": "Back to view",
...
function saveReport(action, payload, msg, repo, form_id) {
  console.log('saveReport',action, payload, msg, repo, form_id);
  console.log(config.httpHost.app[httpHost] + config.api.post + repo + '/' + form_id)
  return false;
}
function saveReportBU(action, payload, msg, repo, form_id) {
  console.log('saveReport',payload);
  $(".btn").prop('disabled', true);
  $.ajax({
    "url": config.httpHost.app[httpHost] + config.api.post + repo + '/' + form_id ,
    "type": "POST",
    "data": payload,
    "headers": {
      "Authorization": "AuthSession " + getCookie(config.default_repo + '.sid'),
...
function updateReport(fid, action, payload, msg, repo, form_id) {
  $(".btn").prop('disabled', true);
  console.log(updateReport , payload)
  $.ajax({
    "url": config.httpHost.app[httpHost] + config.api.get + repo + '/' + form_id + "('" + fid + "')",
    "type": "PATCH",
    "data": payload,
    "headers": {
      "Authorization": "AuthSession " + getCookie(config.default_repo + '.sid'),
    ...
    console.log("PATCH Request Failed: " + textStatus + ", " + error);
...
function processForm(action, repo, form_id, fid) {
 ...
 // let f_data = app.forms[form_id].getData();
  console.log('process any drop zones and add the uploaded file data to the payload.');
  /*
  $('.dropzone').each(function(index){
...
function appInitialize(){$("#user_auth_title").html("<span style=\"margin-left:4px;\">" + cot_login_app.username+ "</span>");}
  ...
  switch(form_id){
      ...
      section = [
        {
          ...
          "rows":[
            {
              "fields":[
                {"id": "VendorNumber", "title": "Vendor Number", "required":true},
                {"id": "VendorName", "title": "Vendor Name", "required":true},
                {"id": "EffectiveDate", "title": "Effective Date", "type":"datetimepicker", "required":true}
              ]
            },
            {
              "fields":[
                {"id": "UserID", "title": "User ID", "required":true},

                {"id": "EmailAddress", "title": "Recovery EMail", "required":true,"validationtype": "Email"},
                {"id": "NewOrderNotification", "title": "New Work Order / Operation", "required":true,"orientation":"horizontal", "type":"radio", "choices":[{"text":"Yes", "value":"Yes"},{"text":"No", "value":"No"}]}
              ]
            },
            {
              "fields": [
                {"id": "AccessKey", "title": "Access Key", "required":true, "type":"password"},
                {"id": "ConfirmAccessKey", "title": "Confirm Access Key", "required":true, "type":"password", validators:{identical:{field:'AccessKey', message:'Access key and Confirm Access Key are not the same'}}}
              ]
            },
            {
              "grid": {
              ...
              "fields":[
                {
                  ...
                  "type": "html",
                  "html": "<button type=\"submit\" class=\"btn btn-success btn-save\" id=\"save\"><span class=\"glyphicon glyphicon-floppy-disk\" aria-hidden=\"true\"></span> " + config.button.saveReport + "</button>",
                  "class": "pull-left"
                }
              ]
            },
          ...
          "rows":[
            ...
            },
            {
              "fields":[
                ...
                {"id": "ShortName", "title": "Short Name", "required":false}
              ]
            },
            {
              "fields":[
                {"id": "City", "title": "City", "required":true},
                {"id": "State", "title": "State", "required":true}
              ]
            },
            {
              "fields":[
              ...
              "fields":[
                {
                  ...
                  "type": "html",
                  "html": `
                          <button type="submit" class="btn btn-success btn-save" id="save"><span class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></span>`  + config.button.saveReport + `</button>
                          <button class="btn btn-danger btn-delete" id="delete"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span> ` + config.button.delete +` </button>                  
                  `,
                  "class": "pull-left"
                }
              ]
      ...
      section = [
        {
          ...
          "rows":[
            {
              "fields":[
                {"id": "SAPPlannerGroup", "title": "SAPPlannerGroup"},
                {"id": "DistrictName", "title": "BU Name", "required":true}
              ]
            },
            {
              "fields":[
                {"id": "PhoneNumber", "title": "BU Phone Number"},
                {"id": "SupervisorName", "title": "Supervisor Name", "required":true}
              ]
            },
            {
              "fields":[
                {"id": "SuperPhoneNumber", "title": "Supervisor Phone", "required":true},
                {"id": "EmailAddress", "title": "Supervisor EmailAddress", "required":true}
              ]
            },
            {
              "fields":[
                {"id": "Description", "title": "Planner Group Description"},
              ]
            }
          ]
        },
    ...
    case'user_status':
      section = [
        {
          "id":"adminSection",
          "title":"User Statuses Code",
          "rows":[
            {
              "fields":[
                {"id": "StatusCode", "title": "Status Code", "required":true},
                {"id": "StatusText", "title": "Status Text", "required":true},
                {"id": "StatusDescription", "title": "Description", "required":true}
              ]
            }
          ]
        },
    ...
    case'bu_it':
      section = [
        {
          "id":"adminSection",
          "title":"Business Unit Invoicing Texts",
          "rows":[
            {
              "fields":[
                {"id": "DistrictName", "title": "BU Name", "required":true}
            ...
            {
              "fields":[
                {"id": "MailAddress", "title": "Mail Address", "type":"textarea", "required":true},
              ]
            },
            {
              "fields":[
                {"id": "EmailAddress", "title": "Email Address", "required":true}
              ]
            },
            {
              "fields":[
                {"id": "MailAttention", "title": "Attention", "required":true},
              ]
            }
          ]
        },
              ...
              "fields":[
                {
                  ...
                  "title": config.button.saveReport,
                  "type": "html",
                  "html": "<button  class=\"btn btn-success btn-save\" id=\"save\"><span class=\"glyphicon glyphicon-floppy-disk\" aria-hidden=\"true\"></span> " + "SAVE ME!" + "</button>",
                  "class": "pull-left"
                }
              ]
            },
      ...
      section = [
        {
          ...
          "rows":[
            {
              "fields":[
                {"id": "name", "title": "Reference Name", "required":true}
              ]
            },
            {
              "fields":[
                {"id": "value", "title": "Value", "required":true},
              ]
            },
            {
              "fields":[
                {"id": "EmailAddress", "title": "Email Address", "required":false}
              ]
            }
          ]
        },
  ...
  switch (formName) {
    case'vendor':
      columnDefs = [
        ...
        { data:'VendorName',    title:'Vendor' , filter:false,sortOrder: "asc" },
        { data:'VendorNumber',   title:'Vendor#' , filter:false,sortOrder: "desc" },
        { data:'EffectiveDate',    title:'Effective Date' , filter:false,sortOrder: "desc" , type:'date'},
        { data:'UserID',    title:'User Name' , filter:false,sortOrder: "desc" }
      ];
      view = "vendor";
      break;
    case'bu':
      columnDefs = [
        { "targets": 0,"checkboxes": {"selectRow": true},"data": "id","createdCell": function (td, cellData, rowData) {}},
        { data:'SAPPlannerGroup',   title:'SAPPlannerGroup' , filter:true,sortOrder: "desc" },
        { data:'DistrictName',   title:'BU Name' , filter:true,sortOrder: "desc" },
        { data:'SupervisorName',    title:'Supervisor Name' , filter:true,sortOrder: "desc" },
      ];
      view = "bu";
      break;
    case'bu_it':
      columnDefs = [
        { "targets": 0,"checkboxes": {"selectRow": true},"data": "id","createdCell": function (td, cellData, rowData) {}},
        { data:'DistrictName',   title:'BU Name' , filter:true,sortOrder: "desc" },
        { data:'EmailAddress',    title:'Email Address' , filter:true,sortOrder: "desc" },
        { data:'MailAttention',    title:'Attention' , filter:true,sortOrder: "desc" },
        //{ data:'MailAddress',    title:'Mail Address' , filter:true,sortOrder: "desc" },
      ];
      view = "bu_it";
      break;
    case'rd':
      columnDefs = [
        { "targets": 0,"checkboxes": {"selectRow": true},"data": "id","createdCell": function (td, cellData, rowData) {}},
        { data:'ReferenceName',   title:'Reference Name' , filter:false,sortOrder: "desc" },
        { data:'TimeDelay',    title:'Time Delay' , filter:false,sortOrder: "desc" },
        { data:'DelayUnit',    title:'Delay Unit' , filter:false,sortOrder: "desc" },
      ];
      view = "rd";
      break;
    case'user_status':
      columnDefs = [
        { "targets": 0,"checkboxes": {"selectRow": true},"data": "id","createdCell": function (td, cellData, rowData) {}},
        { data:'StatusCode',   title:'Status Code' , filter:false,sortOrder: "asc" },
        { data:'StatusText',    title:'Status Text' , filter:false,sortOrder: "desc" },
        { data:'StatusDescription',    title:'Description' , filter:false,sortOrder: "desc" },
      ];
      view = "user_status";
      break;
    default:
...
function registerEvents(){
  ...
  */
  $("#maincontent").off("click", ".btn-deleteSelected").on("click", ".btn-deleteSelected", function (e) {
    ...
    let collectionName = hasher.getHashAsArray()[0]
    bootbox.confirm({
      size: "small",
